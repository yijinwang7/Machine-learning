...
def fetch_dataloader(pkl_dir, dataset_name, batch_size, num_workers, mode):
	...
	y = (int)(len(dataset)/4)
	#random_set = torch.utils.data.random_split(dataset, [y,len(dataset)-y])
	#dataloader = DataLoader(random_set[0],shuffle=True, batch_size=batch_size, num_workers=num_workers)
